...
- **强制「四问确认」闭环**：生成纪要前必须确认「会议类型 + 侧重 + 受众 + 输出形式」，绝不脑补、缺失即标注。
- **双视图输出**：执行摘要（一屏看懂）＋ 完整纪要（可回溯原文锚点）。
- **输出形式可选**：详细 Word（存档）/ 可视化 HTML 单页（卡片式 + 一键导出 PDF·PNG·JPG，分享好看）/ Markdown（贴进知识库），可多选同时产出。
...
```
meeting-minutes-skill/
├── SKILL.md                 # 主流程、四条铁律、四问确认卡片、输出结构
└── references/
    ├── presets.md           # 17 个场景模板的方法论与专属模块
    ├── signals.md           # 自动分类信号词库与置信度规则
    ├── redaction.md         # 对外脱敏规则库与敏感信息清单
    ├── output_formats.md    # 三种输出形式的选择建议与生成要点
    └── visual_template.html # 可视化 HTML 骨架（含导出 PDF/PNG/JPG 按钮）
```
...
- 把会议文字稿复制到对话框。
- 跟 AI 说：「整理成会议纪要」。
- 按它问的四点确认（场景 / 侧重 / 受众 / 输出形式），就出纪要了。

## 输入 / 输出

- **输入**：已转录的会议文字稿（可带说话人标签 / 时间戳，也可裸文本）。
- **输出**：Markdown 会议纪要，可按需同时产出 详细 Word（docx）＋ 可视化 HTML 单页。受众为「双版本」时输出对外脱敏版 ＋ 内部完整版。
